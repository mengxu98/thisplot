pak::pak(
  c(
    ...
    "dichromat", "jaredhuling/jcolors", "thisutils"
  )
)
...
redmonder_pal <- Redmonder::redmonder.pal.info
...
for (pal in all_colors) {
  if (!pal %in% all_colors) {
    thisutils::log_message(
      "Invalid {.val {pal}} Must be one of {.val {all_colors}}",
      message_type = "error"
    )
  }
  if (pal %in% rownames(brewer_pal)) {
    pal_n <- brewer_pal[pal, "maxcolors"]
    pal_category <- brewer_pal[pal, "category"]
    if (pal_category == "div") {
  ...
  } else if (pal %in% rownames(redmonder_pal)) {
    pal_n <- redmonder_pal[pal, "maxcolors"]
    pal_category <- redmonder_pal[pal, "category"]
    if (pal_category == "div") {
