...
* **fix**:
  * Declare `ComplexHeatmap` as an imported dependency so heatmap examples run on clean check systems.
...
* **feat**:
  * `StatPlot()`: Optimized venn plot rendering by using `coord_fixed(ratio = 1, clip = "off")`.
  * Heatmap functions:
